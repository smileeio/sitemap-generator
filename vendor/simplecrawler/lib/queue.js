...
var FetchQueue = function() {
    ...
    /**
     * Tracks the actual number of non-null items in the queue.
     * Used when pruning is enabled to return accurate queue size.
     * @name FetchQueue._itemCount
     * @private
     * @type {Number}
     */
...
/**
 * Prunes a queue item by id to free memory. Sets the slot to null,
 * removes from scanIndex, and decrements item count.
 * @param {Number} id ID of the queue item to be pruned
 */
FetchQueue.prototype.pruneItem = function(id) {
    var queue = this;

    if (id >= 0 && id < queue.length && queue[id] !== null) {
        var url = queue[id].url;
        queue[id] = null;

        if (url && queue._scanIndex[url]) {
            delete queue._scanIndex[url];
        }

        queue._itemCount--;

        // Advance _oldestUnfetchedIndex if we pruned it
        if (id === queue._oldestUnfetchedIndex) {
            for (var i = queue._oldestUnfetchedIndex + 1; i < queue.length; i++) {
                if (queue[i] && queue[i].status === "queued") {
                    queue._oldestUnfetchedIndex = i;
                    return;
                }
            }
            // If no unfetched items remain, set to length
            queue._oldestUnfetchedIndex = queue.length;
        }
    }
};
...
FetchQueue.prototype.max = function(statisticName, callback) {
    ...
    queue.forEach(function(item) {
        if (item && item.fetched && item.stateData[statisticName] > maximum) {
...
FetchQueue.prototype.min = function(statisticName, callback) {
    ...
    queue.forEach(function(item) {
        if (item && item.fetched && item.stateData[statisticName] < minimum) {
...
FetchQueue.prototype.avg = function(statisticName, callback) {
    ...
    queue.forEach(function(item) {
        if (item && item.fetched && Number.isFinite(item.stateData[statisticName])) {
...
FetchQueue.prototype.filterItems = function(comparator, callback) {
    var items = this.filter(function(queueItem) {
        return queueItem && compare(comparator, queueItem);
    });
...
FetchQueue.prototype.getLength = function(callback) {
    // Use _itemCount if it's initialized (will be 0 or more), otherwise fall back to this.length
    var length = this._itemCount !== undefined ? this._itemCount : this.length;

...
var Crawler = function(initialURL) {
    ...
    /**
     * Controls whether to prune completed queue items to free memory.
     * When true, queue items are set to null after fetchcomplete is emitted.
     * @type {Boolean}
     */
    this.pruneCompletedQueueItems = false;
    ...
    var clientRequest = client.request(requestOptions, function(response) {
        if (response.statusCode >= 200 && response.statusCode < 300) {
            ...
            var decodeAndReturnResponse = function(error, responseBuffer) {
                ...
                if (responseBuffer.length >= crawler.maxResourceSize) {
                    return callback(new Error("robots.txt exceeded maxResourceSize"));
                }
    ...
    crawler.queue.update(queueItem.id, {
        ...
        function emitFetchComplete(responseBody, decompressedBuffer) {
            crawler.queue.update(queueItem.id, {
                ...
                // Prune the queue item if pruning is enabled
                // This happens after all listeners have consumed the queueItem
                if (crawler.pruneCompletedQueueItems) {
                    crawler.queue.pruneItem(queueItem.id);
                }
            });
        }
